#!/usr/bin/env bash
# SteadIO - Docker Compose e2e integration test suite
...
echo "============================================================"
echo " SteadIO - Integration Test Suite"
echo "============================================================"
echo ""
...
# Create key via API
KEY_RESP=$(http_body_status \
  -X POST "$CE/api/keys" \
  -H "Content-Type: application/json" \
  -d "{\"teamId\": \"$TEAM_ID\", \"name\": \"E2E Key\"}")
KEY_STATUS=$(echo "$KEY_RESP" | tail -1)
KEY_BODY=$(echo "$KEY_RESP" | head -n -1)
check_http "POST /api/keys returns 201" "201" "$KEY_STATUS"

API_KEY=$(echo "$KEY_BODY" | json_field "d['key']")
KEY_ID=$(echo "$KEY_BODY" | json_field "d['id']")
if [ -n "$API_KEY" ] && [ "$API_KEY" != "None" ]; then
  pass "API key created (id=$KEY_ID)"
else
  fail "API key creation" "unexpected response: $KEY_BODY"
fi

# List keys
LIST_RESP=$(curl -s "$CE/api/keys?teamId=$TEAM_ID")
LIST_COUNT=$(echo "$LIST_RESP" | json_field "len(d.get('keys', []))")
if [ "${LIST_COUNT:-0}" -ge 1 ]; then
  pass "GET /api/keys lists created key"
else
  fail "GET /api/keys" "expected 1 key, got: $LIST_RESP"
fi
...
STATUS=$(http_status \
  -H "X-SteadIO-Key: sk-not-a-real-key-$(rand_hex 8)" \
  -H "Content-Type: application/json" \
  -d '{"model":"gpt-4o-mini","messages":[{"role":"user","content":"hello"}]}' \
  "$PROXY/openai/chat/completions")
check_http "invalid API key rejected" "401" "$STATUS"

# Revoke key — verify API returns 200
STATUS=$(http_status -X DELETE "$CE/api/keys/$KEY_ID")
check_http "DELETE /api/keys/:id revokes key" "200" "$STATUS"
# Note: revoked keys may still be accepted by the proxy for up to 5 minutes
# while the Redis auth cache holds the previous resolution. This is expected.

# Re-create key for remaining tests
NEW_KEY_RESP=$(curl -s -X POST "$CE/api/keys" \
  -H "Content-Type: application/json" \
  -d "{\"teamId\": \"$TEAM_ID\", \"name\": \"E2E Key 2\"}")
API_KEY=$(echo "$NEW_KEY_RESP" | json_field "d['key']")
...
# Create a fresh team + key for budget isolation via API
BTEAM_ID="e2e-bt-$(rand_hex 8)"
BUDGET_KEY_RESP=$(curl -s -X POST "$CE/api/keys" \
  -H "Content-Type: application/json" \
  -d "{\"teamId\": \"$BTEAM_ID\", \"name\": \"Budget Key\"}")
BKEY=$(echo "$BUDGET_KEY_RESP" | json_field "d['key']")
BKEY_ID=$(echo "$BUDGET_KEY_RESP" | json_field "d['id']")
