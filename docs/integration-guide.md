...
Point your existing LLM clients at SteadIO by changing one or two lines. The proxy is fully OpenAI-compatible.

## How It Works

SteadIO acts as a transparent proxy:

1. Your client sends requests to SteadIO instead of directly to OpenAI or Anthropic.
2. SteadIO attaches SteadIO auth headers, records token usage, checks budgets, and checks for runaway patterns.
3. SteadIO forwards the request to the upstream provider and streams the response back.
4. Your application receives the exact same response format it would from the provider directly.

Your existing provider API keys stay in your code. SteadIO reads `X-SteadIO-Key` for attribution; it never stores or proxies your provider credentials.

## Provider Routes

| Provider  | SteadIO Route Prefix   | Original Base URL              | Status    |
|-----------|--------------------------|--------------------------------|-----------|
...
client = OpenAI(
    ...
    default_headers={
        "X-SteadIO-Key": "el_mycompany_abc123",
        "X-Agent-Id": "my-agent",
...
SteadIO automatically injects `stream_options: { include_usage: true }` to capture token counts from streamed responses.

### Environment-variable approach

Keep SteadIO configuration out of your code:
...
client = OpenAI(
    base_url=os.environ.get("STEADIO_PROXY_URL", "http://localhost:3001/openai"),
    api_key=os.environ["OPENAI_API_KEY"],
    default_headers={
        "X-SteadIO-Key": os.environ["STEADIO_API_KEY"],
        "X-Agent-Id": os.environ.get("STEADIO_AGENT_ID", "default-agent"),
    },
)
```
...
const client = new OpenAI({
  ...
  defaultHeaders: {
    ...
    "X-Agent-Id": process.env.STEADIO_AGENT_ID ?? "my-agent",
  },
});
