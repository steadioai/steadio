...
**`throttle`**: Requests still go through but SteadIO logs an alert. (Hard throttling is on the roadmap.)

**`kill`**: SteadIO blocks all requests from the agent or team once the cap is hit, returning HTTP 429 with a `budget_exceeded` error until the period resets.
...
SteadIO can send webhook or Slack alerts when budget thresholds are hit or runaways are detected.
