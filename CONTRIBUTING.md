# Contributing to Elevation Networks
...
```bash
pnpm --filter @elevation/proxy dev        # port 3001
pnpm --filter @elevation/cost-engine dev  # port 3002
pnpm --filter @elevation/dashboard dev    # port 5173
```
...
pnpm --filter @elevation/proxy test # single package
```
...
packages/proxy  (Hono, port 3001)
    │  1. Validate X-Elevation-Key against DB (Redis cache, 5 min TTL)
...
packages/cost-engine  (Hono + Drizzle + TimescaleDB, port 3002)
    │  1. Calculate cost using provider pricing tables in @elevation/shared
