...
#   3. Seed demo cost data (no real API keys required)
#   4. Print dashboard URL and sample curl commands
...
seed_event() {
  ...
  curl -sf -X POST "$COST_ENGINE_URL/internal/proxy-events" \
    ...
    -d "{
      \"requestId\": \"demo-$(openssl rand -hex 8)\",
      \"agentId\": \"$agent_id\",
      \"teamId\": \"$team_id\",
...
step "Seeded 12 sample cost events across 2 teams and 6 agents"
...
if [ -n "$DEMO_KEY" ]; then
  ...
  info "Key creation failed — use 'curl -X POST $COST_ENGINE_URL/api/keys -d ...' to create one manually"
fi
...
echo ""
banner "Sample proxy request (requires a real provider API key):"
echo ""
echo -e "  ${CYAN}export STEADIO_KEY='${DEMO_KEY}'"
echo -e "  curl $PROXY_URL/openai/chat/completions \\"
echo -e "    -H 'Content-Type: application/json' \\"
echo -e "    -H \"X-SteadIO-Key: \$STEADIO_KEY\" \\"
